			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						var scenarioWinningNumbers = getWinningNumbers(scenario);
						...
						var gridCanvasYourHeight = cellSizeY + 2 * cellMargin; //gridRows * cellSizeY + 2 * cellMargin;
						var gridCanvasYourWidth  = cellSizeX + 2 * cellMargin; //gridCols * cellSizeX + 2 * cellMargin;
						...
						function showYourNums(A_strCanvasId, A_strCanvasElement, A_arrGrid)
						{
							...
							for (var gridRow = 0; gridRow < gridRows; gridRow++)
							{
								...
								for (var gridCol = 0; gridCol < gridCols; gridCol++)
								{
									...
									symbCell = A_arrGrid[tempNum].playItem;
									prizeCell = A_arrGrid[tempNum].playPrize;
									...
									cellX         = 0; //gridCol * cellSizeX;
									cellY         = 0; //gridRow * cellSizeY;

									canvasCtxStr = 'canvasContext' + A_strCanvasElement + tempNum.toString();
									canvasIdStr  = A_strCanvasId + tempNum.toString();

									r.push('<td>');

									r.push('<canvas id="' + canvasIdStr + '" width="' + gridCanvasYourWidth.toString() + '" height="' + gridCanvasYourHeight.toString() + '"></canvas>');
									r.push('<script>');
									r.push('var ' + A_strCanvasElement + tempNum.toString() + ' = document.getElementById("' + canvasIdStr + '");');
									r.push('var ' + canvasCtxStr + ' = ' + A_strCanvasElement + tempNum.toString() + '.getContext("2d");');
									r.push(canvasCtxStr + '.textAlign = "center";');
									r.push(canvasCtxStr + '.textBaseline = "middle";');

									if ((gridCol > 0) && (gridCol < 5))
									{
										r.push(canvasCtxStr + '.strokeRect(' + (cellX + cellMargin).toString() + ', ' + (cellY + cellMargin + 0.5).toString() + ', ' + (cellSizeX).toString() + ', ' + cellSizeY.toString() + ');');
									}
									else
									{
										r.push(canvasCtxStr + '.strokeRect(' + (cellX + cellMargin + 4.5).toString() + ', ' + (cellY + cellMargin + 0.5).toString() + ', ' + (cellSizeX - 8).toString() + ', ' + cellSizeY.toString() + ');');
									}
									r.push(canvasCtxStr + '.fillStyle = "' + boxColourStr + '";');
									if ((gridCol > 0) && (gridCol < 5))
									{
										r.push(canvasCtxStr + '.fillRect(' + (cellX + cellMargin + 1).toString() + ', ' + (cellY + cellMargin + 1.5).toString() + ', ' + (cellSizeX -2).toString() + ', ' + (cellSizeY - 2).toString() + ');');
									}
									else
									{
										r.push(canvasCtxStr + '.fillRect(' + (cellX + cellMargin + 6.5).toString() + ', ' + (cellY + cellMargin + 1.5).toString() + ', ' + (cellSizeX - 12).toString() + ', ' + (cellSizeY - 2).toString() + ');');
									}
									r.push(canvasCtxStr + '.fillStyle = "' + textColourStr + '";');
									if (gridCol > 0)
									{
									...
									else
									{
										...
										r.push(canvasCtxStr + '.fillText("' + symbCell + '", ' + (cellX + cellTextX).toString() + ', ' + (cellY + cellTextY1 + 10).toString() + ');');
										r.push(canvasCtxStr + '.fillStyle = "' + textColourStr + '";');
									}
									r.push('</script>');
									r.push('</td>');
						...
						// Headings
						r.push('<tr>');
						r.push('<td colspan="5">' + getTranslationByName("winningNumbers", translations) + '</td>');
						r.push('</tr>');
						...
						r.push('<tr>');
						r.push('<td colspan="1" /td>');
						r.push('<td colspan="4" align="center">' + getTranslationByName("yourNumbers", translations) + '</td>');
						r.push('<td colspan="1">' + getTranslationByName("jokerNumber", translations) + '</td>');
						r.push('</tr>');
						r.push('<tr>');
					...
					function getYourNumbers(scenario)
					{
						...
						for(var j = 1; j < 4; ++j)
						{
							outcomeData = scenario.split("|")[j];
							outcomePairs = outcomeData.split(",");
							for(var i = 0; i < outcomePairs.length; ++i)
							{
								itemData = outcomePairs[i].split(":");
								objItem = {playItem: itemData[0], playPrize: itemData[1]};
								result.push(objItem);
							}							
						}
						return result;
